...
Thread_Image_Show::Thread_Image_Show(){
...
void Thread_Image_Show::AdaptiveThereshold(cv::Mat src, cv::Mat &dst)
{
...
void Thread_Image_Show::CountAdaptive(cv::Mat frame, cv::Mat& binaryImg, cv::Mat& inBinaryImg, float& result1, float& result2)
{
...
void Thread_Image_Show::CountOtsu(cv::Mat frame, cv::Mat& binaryImg, float& result)
{
    //frame = frame(Rect(frame.cols * 2 / 5, frame.rows * 2 / 5, frame.cols * 1 / 5, frame.rows * 1 / 5));//ẄØÈḂROI
...
void Thread_Image_Show::filtrate(cv::Mat frame, cv::Mat& result_binary, float& result)
{
        qDebug()<< "show_filtrate";
        float result_Otsu = 0;
...
int Thread_Image_Show::avgGray(cv::Mat img)
{
    int pxl = 0;
    int sum = 0;
...
float Thread_Image_Show::count(cv::Mat src)
{
    int black = 0;
    int sum = 0;
    float result = 0;
...
//计算网点间距与角度
void Thread_Image_Show::count_distance_angle(Mat src, float binary_result, float &mean_distance, float &mean_angle)
{
    /***************************************提取轮廓**********************************************/
    Mat result_binary2;
    if (binary_result < 50)
...
void Thread_Image_Show::run()
{
    ...
    if(!cvRawImg.empty()){
        ...
        GaussianBlur(local_frame,local_frame,Size(3,3),3,3);
      //  filtrate(local_frame, cvBinImg, result);
      //  count_distance_angle(cvBinImg, result, distance, angle);
        if (distance != 0)
        {
            amount_points = floor(25.4 / (proportion*distance)) + 1;
        }
        emit DisplayContinuImageSignal(result);
    }
    else{
...
void Thread_Image_Show::stopimmedqiately(){
    QMutexLocker locker(&m_lock);
    m_isCanRun = false;
